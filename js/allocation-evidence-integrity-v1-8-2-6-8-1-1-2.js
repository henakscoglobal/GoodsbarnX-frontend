...
(function(){
  ...
  var EXPECTED={build:"GBX-V1.8.2.6.8.1.1.2-EVIDENCE-INTEGRITY-20260916",authVersion:"V1.8.2.6.6.4",allocationVersion:"V1.8.2.6"};
  ...
  async function run(){
    ...
    try{
      ...
      // V1.8.2.6.8.1.1.2: preserve runtime identity while synchronizing with the existing authentication resolver.
      ...
      var inquiryForensics=[];
      inquiries.forEach(function(i){
        if(!validId(i.id)){
          inquiryInvalid++;
          inquiryForensics.push({inquiry_id:safe(i.id),item:safe(i.item),buyer_id:safe(i.buyer_id),inquirer_id:safe(i.inquirer_id),product_id:safe(i.product_id),quantity:safe(i.quantity),status:safe(i.status),field:"id",predicate:"validId(id)",raw_value:safe(i.id),normalized_value:validId(i.id)?String(i.id).trim():"",reason:"Inquiry id is missing or blank."});
        }
        if(i.quantity==null||String(i.quantity).trim()===""){
          inquiryMissing++;
        } else if(!numeric(i.quantity)||Number(i.quantity)<=0){
          inquiryInvalid++;
          inquiryForensics.push({inquiry_id:safe(i.id),item:safe(i.item),buyer_id:safe(i.buyer_id),inquirer_id:safe(i.inquirer_id),product_id:safe(i.product_id),quantity:safe(i.quantity),status:safe(i.status),field:"quantity",predicate:"numeric(quantity) && Number(quantity) > 0",raw_value:safe(i.quantity),normalized_value:numeric(i.quantity)?String(Number(i.quantity)):"NaN",reason:!numeric(i.quantity)?"Quantity is not numeric.":"Quantity must be greater than zero."});
        }
        var st=String(i.status||"").trim().toLowerCase();
        if(!st)inquiryMissing++;
        else if(["open","pending","closed","resolved","completed"].indexOf(st)===-1){
          inquiryUnsupported++;
          inquiryForensics.push({inquiry_id:safe(i.id),item:safe(i.item),buyer_id:safe(i.buyer_id),inquirer_id:safe(i.inquirer_id),product_id:safe(i.product_id),quantity:safe(i.quantity),status:safe(i.status),field:"status",predicate:"supported inquiry status",raw_value:safe(i.status),normalized_value:st,reason:"Inquiry status is outside the diagnostic-supported status set."});
        }
        if(i.buyer_id!=null&&i.inquirer_id!=null&&String(i.buyer_id)!==String(i.inquirer_id)){
          inquiryAmbiguous++;
          inquiryForensics.push({inquiry_id:safe(i.id),item:safe(i.item),buyer_id:safe(i.buyer_id),inquirer_id:safe(i.inquirer_id),product_id:safe(i.product_id),quantity:safe(i.quantity),status:safe(i.status),field:"buyer_id/inquirer_id",predicate:"buyer_id === inquirer_id when both are present",raw_value:safe(i.buyer_id)+" / "+safe(i.inquirer_id),normalized_value:String(i.buyer_id)+" / "+String(i.inquirer_id),reason:"Buyer attribution fields conflict."});
        }
      });
