...
console.log('currentUser role:', typeof currentUser !== 'undefined' && currentUser ? currentUser.role : 'no user');
...
// ==========================================================================
// DISTRIBUTOR — MY TRADE RELATIONSHIPS (WITH DEBUG)
// ==========================================================================

async function loadMyTradeRelationships() {
  console.log('\n=== 🔍 DEBUG: loadMyTradeRelationships called ===');
  
  const container = document.getElementById("my-relationships-list");
  console.log('1. Container #my-relationships-list exists:', !!container);
  
  if (!container) {
    console.error('❌ CRITICAL: Container #my-relationships-list NOT FOUND');
    console.log('Available IDs:', Array.from(document.querySelectorAll('[id]')).map(el => el.id));
    return;
  }
  
  console.log('2. currentUser:', currentUser);
  console.log('3. currentUser.role:', currentUser?.role);
  console.log('4. window.sb:', !!window.sb);
  
  if (!currentUser || currentUser.role !== "distributor") {
    console.error('❌ User is not distributor. Role:', currentUser?.role);
    container.innerHTML = `<div class="error-message" style="padding:20px; color:red;">
      Debug: User role is "${currentUser?.role || 'undefined'}", expected "distributor"
    </div>`;
    return;
  }
  ...
  try {
    console.log('6. Querying trade_relationships...');
    console.log('7. Distributor ID:', currentUser.id);
    ...
    console.log('8. Query result:', { data: relationships, error: relationshipsError });
    ...
    if (!relationships || relationships.length === 0) {
      console.log('9. No relationships found');
      container.innerHTML = inviteButtonHtml + `
    ...
    console.log(`10. Found ${relationships.length} relationships`);
    ...
    console.log('11. Buyer IDs:', buyerIds);
    console.log('12. Relationship IDs:', relationshipIds);
    ...
    console.log('13. Promise.allSettled results:', {
      buyers: buyersResult.status,
      trust: trustResult.status,
      loyalty: loyaltyResult.status,
    ...
    results.forEach((result, index) => {
      if (result.status === 'rejected') {
        console.error(`❌ Failed to load ${resultNames[index]} data:`, result.reason);
      }
    });
    ...
    console.log('14. Maps built successfully');
    console.log('15. Buyer map:', buyerMap);
    console.log('16. Terms map:', termsMap);
...
async function initRelationshipLayer() {
  console.log('\n=== 🔍 DEBUG: initRelationshipLayer called ===');
