        ...
        <div class="product-item">
          ...
          <div style="display:inline-block; width:calc(100% - 80px);">
            ...
            <div style="margin-top:8px;">
              <button class="btn btn-success" data-product-id="${productId}" data-distributor-id="${distributorIdSafe}" onclick="addToCart(this.dataset.productId, ${JSON.stringify(String(product.name))}, ${Number(finalPrice) || 0}, this.dataset.distributorId, ${JSON.stringify(String(distributorName))})">Add to Cart</button>
            </div>
          </div>
        </div>
      `;
...
async function loadMyTradeRelationships() {
  ...
  if (!window.sb) {
    container.innerHTML = '<div class="error-message">System initialization error. Please refresh the page.</div>';
    return;
  }
  ...
  try {
    ...
    const [buyersResult, trustResult, loyaltyResult, termsResult, disputesResult] = await Promise.allSettled([
    ...
    trustRows.forEach(row => { if (row && row.relationship_id) trustMap[row.relationship_id] = row; });
    loyaltyRows.forEach(row => { if (row && row.relationship_id) loyaltyMap[row.relationship_id] = row; });
    termsRows.forEach(row => { if (row && row.buyer_id) termsMap[row.buyer_id] = row; });
    disputeRows.forEach(dispute => {
      if (dispute && dispute.relationship_id) {
        if (!disputesMap[dispute.relationship_id]) disputesMap[dispute.relationship_id] = [];
...
async function loadMyAgentRelationships() {
  ...
  const { data: relationships } = await safeSupabaseQuery(
    (sb) => sb.from("trade_relationships").select("*").in("id", relationshipIds),
    [],
  ...
  const [buyersResult, distributorsResult, trustResult] = await Promise.allSettled([
    window.sb.from("buyer_profiles").select("id, name, profiles(full_name, phone)").in("id", buyerIds),
    window.sb.from("distributor_profiles").select("id, business_name, location, market").in("id", distributorIds),
    window.sb.from("relationship_trust").select("relationship_id, trust_score, completed_orders, disputed_orders").in("relationship_id", relationshipIds)
  ]);
  ...
  assignments.forEach(assignment => { if (assignment && assignment.relationship_id) assignmentMap[assignment.relationship_id] = assignment; });
    ...
    <div class="manifest" style="padding:6px 16px;">
      ${methods.map((method, index) => `
        <div style="padding:10px 0; ${index < methods.length - 1 ? "border-bottom:1px dashed var(--line-dark);" : ""} display:flex; justify-content:space-between; align-items:center;">
          ...
          ${method.transaction_limit != null ? `<span style="font-size:11px; color:rgba(18,21,28,0.5);">Limit ${relationshipMoney(method.transaction_limit)}</span>` : ""}
        </div>
      `).join("")}
    </div>
  `;
...
function openEditTermsModal(relationshipId) {
  ...
  if (creditEnabledEl) creditEnabledEl.value = cached.creditEnabled ? "true" : "false";
  if (creditLimitEl) creditLimitEl.value = cached.creditLimit;
  if (creditDaysEl) creditDaysEl.value = cached.creditDays;
...
async function searchBuyersForInvite() {
  ...
  resultsEl.innerHTML = buyers.map(buyer => {
    ...
    return `
      <div class="manifest" style="padding:12px; cursor:pointer;" data-buyer-id="${relationshipEscapeAttribute(buyer.id)}" data-buyer-name="${relationshipEscapeAttribute(name)}" onclick="inviteBuyerToRelationship(this.dataset.buyerId, this.dataset.buyerName)">
...
function renderRelationshipActions(relationshipId, status) {
  ...
  return `
    <div style="margin-top:10px; display:flex; gap:8px; flex-wrap:wrap;">
      ${actions.map(action => `
        <button class="btn btn-outline" data-relationship-id="${relationshipEscapeAttribute(relationshipId)}" data-new-status="${relationshipEscapeAttribute(action.newStatus)}" onclick="updateRelationshipStatus(this.dataset.relationshipId, this.dataset.newStatus)">
...
async function searchAgentsForAssignment() {
  ...
  const { data: agents, error } = await safeSupabaseQuery(
    (sb) => sb.from("agent_profiles").select("id, profiles(full_name, phone)").limit(20),
    [],
  ...
  resultsEl.innerHTML = filtered.map(agent => {
    ...
    return `
      <div class="manifest" style="padding:12px; cursor:pointer;" data-agent-id="${relationshipEscapeAttribute(agent.id)}" data-agent-name="${relationshipEscapeAttribute(name)}" onclick="assignAgentToRelationship(this.dataset.agentId, this.dataset.agentName)">
        <div class="m-name">${relationshipEscapeHtml(name)}</div>
  ...
  listEl.innerHTML = methods.map(method => `
    <div class="manifest" style="padding:10px 14px;">
      ...
      <div class="action-buttons">
        ${!method.is_default ? `<button class="btn btn-outline" data-method-id="${relationshipEscapeAttribute(method.id)}" onclick="makePaymentMethodDefault(this.dataset.methodId)">Make Default</button>` : ""}
        <button class="btn ${method.is_active ? "btn-danger" : "btn-success"}" data-method-id="${relationshipEscapeAttribute(method.id)}" data-new-active="${!method.is_active}" onclick="togglePaymentMethodActive(this.dataset.methodId, this.dataset.newActive === 'true')">${method.is_active ? "Deactivate" : "Activate"}</button>
      </div>
    </div>
  `).join("");
}
        ...
        <div class="manifest-top">
          <div>
            ...
            <div class="m-loc">
              ...
              ${preference.negotiated_unit_price != null ? ` · Negotiated: ${relationshipMoney(preference.negotiated_unit_price)}` : ""}
            </div>
          </div>
        </div>
        <div class="action-buttons">
          <button class="btn btn-outline" data-pref-id="${relationshipEscapeAttribute(preference.id)}" onclick="setNegotiatedPrice(this.dataset.prefId)">${preference.negotiated_unit_price != null ? "Change" : "Set"} Price</button>
          <button class="btn btn-danger" data-pref-id="${relationshipEscapeAttribute(preference.id)}" onclick="removeProductPreference(this.dataset.prefId)">Remove</button>
        </div>
      </div>
    `;
  }).join("");
...
async function searchDistributorProductsForPreference() {
  ...
  resultsEl.innerHTML = products.map(product => `
    <div class="manifest" style="padding:10px 14px; cursor:pointer;" data-product-id="${relationshipEscapeAttribute(product.id)}" onclick="addProductPreference(this.dataset.productId)">
      <div class="m-name">${relationshipEscapeHtml(product.name)}</div>
      <div class="m-loc">${relationshipEscapeHtml(product.sku || "No SKU")} · ${relationshipMoney(product.price || 0)}</div>
    </div>
  `).join("");
}
...
async function searchBuyersForAgentReferral() {
  ...
  resultsEl.innerHTML = buyers.map(buyer => {
    const name = buyer.name || buyer.profiles?.full_name || "Buyer";
    return `
      <div class="manifest" style="padding:12px; cursor:pointer;" data-buyer-id="${relationshipEscapeAttribute(buyer.id)}" data-buyer-name="${relationshipEscapeAttribute(name)}" onclick="createAgentReferredBuyerRelationship(this.dataset.buyerId, this.dataset.buyerName)">
        <div class="m-name">${relationshipEscapeHtml(name)}</div>
        <div class="m-loc">
          ${relationshipEscapeHtml(buyer.location || "")}
          ${buyer.profiles?.phone ? " · " + relationshipEscapeHtml(buyer.profiles.phone) : ""}
